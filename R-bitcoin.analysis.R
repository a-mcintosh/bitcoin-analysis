...
one_block_time <- 10 * 60	#A well known bitcoin number
temp_vetted <- bitcoin_lab[ bitcoin_lab[0:10007,"calctime"] < 2*6*one_block_time, "nHeight" ]
vetted <- temp_vetted[ temp_vetted < 10007 ]
...
plot( 1+x, nblock*y, col="black", pch=3, ylab="Block Count", xlab="MedianTimePast (half-block units)", ylim=c(0, y_lim));

breaks <- seq(0, 15*one_block_time, one_block_time/2)
...
plot( 1+x, nblock*y, col="black", pch=3, ylab="Block Count", xlab="MedianTimePast (vetted)", ylim=c(0,y_lim));
